""" MiniMaxGame: This gives the next White move for a Mid/End game using MiniMax"""
...
def GenerateMovesMidgameEndgame(L:list):
    count=collections.Counter(L)
    if count["W"]==3:
        return GenerateHopping(L)
    else:
        return GenerateMove(L)


def staticEstimateMidgameEndgame(L):
    ...
    p = GenerateMovesMidgameEndgame(l1)

    numBlackMoves = len(p)
    l2=copy.deepcopy(L)
    q = GenerateMovesMidgameEndgame(l2)
    numWhiteMoves = len(q)
    ...
    if numBlack <= 2:
        ...
        return(1000 * (numWhite - numBlack) - numBlackMoves + numWhiteMoves)



def MiniMaxGame(L, depth, MaximizingPlayer, ct):
    if depth == 0:
        ct+=1
        l= staticEstimateMidgameEndgame(L)
        return (l, ct, L)

    if MaximizingPlayer=="W":

        maxEva = -math.inf
        l2 = GenerateMovesMidgameEndgame(L)
        n=l2[0]
        for child in l2:
            eva, c, new_n = MiniMaxGame(child, depth - 1, "B", ct)
            ct=c
            if eva > maxEva:
                n = child
                maxEva = eva
        ...
        for i in range(len(l1)):
            if l1[i] =="M":
                l1[i]="B"
        l3 = GenerateMovesMidgameEndgame(l1)
        for list in l3:
        ...
        n=l3[0]
        for child in l3:
            eva,c ,q= MiniMaxGame(child, depth - 1, "W",ct)
            ct=c
            if eva < minEva:
                n = child
                minEva=eva
...
if __name__ == "__main__":
    ...
    # file_location2 = "/Users/monik/PycharmProjects/AI_Project/output8.txt"
