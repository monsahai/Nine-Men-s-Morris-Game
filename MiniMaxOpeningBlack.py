""" MiniMaxOpeningBlack: This gives the next Black move for a Opening game using MiniMax"""
...
# def MiniMaxOpening(L, depth:int, MaximizingPlayer:bool, ct,n):
#
#     if depth == 0:
#         return (staticEstimate(L,ct)[0],staticEstimate(L,ct)[1],n)
#
#     if MaximizingPlayer:
#         maxEva = -math.inf
#         l2 = GenerateMovesOpening(L)
#         for child in l2:
#             eva,ct,n = MiniMaxOpening(child, depth - 1, False,ct,n)
#             if eva>maxEva:
#                 n=child
#                 maxEva=eva
#         return (maxEva,ct,n)
...
#                     list[i]="B"
#         for child in l3:
#             eva,ct ,n= MiniMaxOpening(child, depth - 1, True,ct,n)
#             if eva<minEva:
#                 minEva=eva
#             # print(minEva)
#         return (minEva,ct,n)
...
def MiniMaxOpening(L, depth:int, MaximizingPlayer:bool, ct,n):
    ...
    if MaximizingPlayer:
        ...
        n = l2[0]
        for child in l2:
            eva,c,new_n = MiniMaxOpening(child, depth - 1, False,ct,n)
            ct=c
            if eva>maxEva:
                n=child
                maxEva=eva
        ...
        n = l3[0]
        for child in l3:
            eva,c ,q= MiniMaxOpening(child, depth - 1, True,ct,n)
            ct=c
            if eva<minEva:
                minEva=eva
                n=child
...
if __name__ == "__main__":
    ...
    # file_location2 = "/Users/monik/PycharmProjects/AI_Project/output5.txt"
    ...
    for i in range(len(l1)):
        if l1[i] == 'M':
            l1[i] = 'B'
    # depth = 2
    print("MinMax Opening Game:")
    m,n,l3 = MiniMaxOpening(l1, depth, True,0,[])
    l2 = copy.deepcopy(l3)
